...
The system SHALL allow authenticated employees to record check-in (I) and check-out (O) by clicking a single button. The system SHALL INSERT a record into `WiseEye.dbo.CheckInOut` with `Source='PC'` and the configured `MachineNo`. The system SHALL evaluate remote-risk before every punch action and SHALL block the punch when the remote-risk level is high at the time of the attempt.
